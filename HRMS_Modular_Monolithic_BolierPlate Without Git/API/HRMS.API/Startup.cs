        ...
        public void Configure(WebApplication app, IWebHostEnvironment env, IConfiguration configuration)
        {
            ...
            _ = Task.Run(async () =>
            {
                try
                {
                    ...
                    {
                        var context = scope.ServiceProvider.GetRequiredService<HRMS.Core.Postgres.Data.PostgresDbContext>();
                        await HRMS.API.Data.HrmsDataSeeder.SeedAsync(context);
                        Console.WriteLine("PostgreSQL database seeded successfully");
                    }
                }
                catch (Exception ex)
                {
