...
const roles: { role: UserRole; label: string; icon: React.ComponentType<{ size?: number }>; color: string; desc: string; isOnboarding?: boolean }[] = [
  { role: 'Employee', label: 'Employee', icon: User, color: '#0d9488', desc: 'Sarah Johnson' },
  { role: 'Employee', label: 'New Joiner', icon: User, color: '#7c3aed', desc: 'Alex Rivera (Onboarding)', isOnboarding: true },
...
export function RoleSwitcher() {
  const { activeRole, setRole, isOnboarding, setIsOnboarding } = useHrmsStore();
  const [open, setOpen] = useState(false);
  const ref = useRef<HTMLDivElement>(null);
  const current = roles.find(r => r.role === activeRole && (r.isOnboarding ? isOnboarding : !isOnboarding)) || roles[0];
    ...
    <div ref={ref} style={{ position: 'relative' }}>
      ...
      >
        ...
        <span style={{ fontSize: '0.75rem', fontWeight: 700, color: current.color }}>
          {current.isOnboarding ? 'New Joiner' : current.role}
        </span>
      ...
      {open && (
        <div style={{
          ...
          {roles.map(r => {
            const isSelected = activeRole === r.role && (r.isOnboarding ? isOnboarding : !isOnboarding);
            return (
              <button
                key={r.label}
                onClick={() => {
                  setRole(r.role);
                  setIsOnboarding(!!r.isOnboarding);
                  setOpen(false);
                  window.location.href = r.isOnboarding ? '/onboarding' : '/home';
                }}
                style={{
                  width: '100%', display: 'flex', alignItems: 'center', gap: 10,
                  padding: '10px 12px', background: isSelected ? `${r.color}0e` : 'transparent',
                  border: 'none', cursor: 'pointer', textAlign: 'left',
                  transition: 'background 0.15s',
                }}
                onMouseEnter={e => { if (!isSelected) (e.currentTarget as HTMLButtonElement).style.background = '#f8fafc'; }}
                onMouseLeave={e => { (e.currentTarget as HTMLButtonElement).style.background = isSelected ? `${r.color}0e` : 'transparent'; }}
              >
                <span style={{
                ...
                <div style={{ minWidth: 0 }}>
                  <p style={{ fontSize: '0.8125rem', fontWeight: 700, color: isSelected ? r.color : 'var(--foreground)', margin: 0 }}>{r.label}</p>
